...
from models.dyhead import *  ## 65 dyhead
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        #self.dyhead = nn.Sequential(*[DyHeadBlock(ch[0]) for i in range(2)]) ###change here#############dyhead


    def forward(self, x):
        # for dyhead_layer in self.dyhead:     ##########dyhead
        #     x = dyhead_layer(x)
        z = []  # inference output
        logits_ = []  # 修改---1
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                GSConv, GSConvns, C3_DCN}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3_Faster, C2f, C2f_Faster, C3_DCN}:
        ...
        # TODO: channel, gw, gd
            ...
            c2 = m.feature_info.channels()
                   ...
                   efficientformerv2_s0, efficientformerv2_s1, efficientformerv2_s2, efficientformerv2_l, SwinTransformer_Tiny}:
            m = m(*args)
            c2 = m.channel
